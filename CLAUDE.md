...
- **Auto-retry after a mostly-failed run** — when a run ends with `scrapeErrors >= ceil(groupsChecked/2)` (half or more groups failed to load), `scheduleRetry()` writes `data/retry_pending.json`. The next hourly PM2 fire sees `hasPendingRetry()` true and runs even outside the configured run times (`isRetryRun`), recovering within the hour instead of waiting for the next slot. Capped at `MAX_AUTO_RETRIES = 2`; flag expires after `RETRY_EXPIRY_HOURS = 12` (ignores stale overnight flags). A successful run (`< half` failed) calls `clearRetryFlag()`. Weekend + business-hours guards run earlier, so retries never fire at night or on weekends. Session-expiry and checkpoint failures `return` before the summary, so they never schedule a retry (they need manual intervention).
