...
- `data/last_success_at.json` — timestamp of last SUCCESSFUL run (groups scraped); powers the watchdog dead-man's switch (gitignored)
- `data/retry_pending.json` — auto-retry flag dropped by a failed run; honored by the next hourly fire (gitignored)
...
- **Catch-up / make-up mechanism** — PM2 fires hourly (Mon–Fri). `hasMissedScheduledRun()` compares `data/last_success_at.json` (NOT last_run_at.json, which updates on every fire incl. failures — that defeated catch-up exactly during outages) against the configured run times; if the gap since the last *successful* scrape exceeds the largest interval between scheduled run times (`maxRunGapHours`), it runs as a catch-up regardless of the current hour. **Scroll depth scales with `hoursSinceLastSuccess`** so a make-up run scrolls back far enough to recover posts from the missed window: normal=5, >8h=8, >24h=12, >48h=20 passes (dedup drops anything already seen). A make-up run (`isMakeupRun` = `(isCatchupRun || isRetryRun)` AND gap ≥ maxRunGap) announces "Catching Up On Missed Runs" to Teams — but only AFTER a successful login, so a still-offline retry that cannot connect does not announce a catch-up it cannot perform. Normal scheduled runs are never treated as make-ups (the `isCatchupRun || isRetryRun` gate excludes them). `last_run_at.json` is still written each fire but is now vestigial (no longer read).
- **Auto-retry + failure alert** — covers two failure shapes: (a) **ran but mostly failed** — `groupsChecked>0 && scrapeErrors >= ceil(groupsChecked/2)` (mid-run network blip / FB block); (b) **failed before scraping** — `groupsChecked===0 && errors.length>0`, i.e. the run threw at `loadConfig` or `verifyLogin` because the internet / Google Sheets / Facebook was unreachable (DNS `ENOTFOUND`, `page.goto` timeout). Both call `scheduleRetry()` (writes `data/retry_pending.json`) so the next hourly PM2 fire runs even outside the configured run times (`isRetryRun`), recovering within the hour. Capped at `MAX_AUTO_RETRIES = 2`; flag expires after `RETRY_EXPIRY_HOURS = 12`. A successful run (`groupsChecked>0` and not mostly-failed) calls `clearRetryFlag()`. **Alert throttle:** `retryFlagExistedAtStart` (captured at run start) gates the Teams alert so a multi-hour outage sends ~one alert per streak, not one per hour. Legitimate early exits (wrong hour, weekend, monitor disabled) `return` before the end-of-run block, so `errors.length` cleanly distinguishes a real failure from a no-op. Session-expiry (3+ login redirects) and checkpoint `return`/handle separately, so they never auto-retry (need manual intervention). **Note:** `last_run_at.json` is rewritten on every fire (incl. failed/no-op), so `hasMissedScheduledRun()` catch-up rarely triggers under hourly PM2 — the retry flag is the real failure-recovery path.
...
- **Watchdog dead-man's switch** — the per-run alert only fires when a run actually executes; it cannot catch failures where no run happens (PM2 not firing, machine asleep, process gone). The watchdog (every 2h) closes this by watching for the *absence of success*: index.js writes `data/last_success_at.json` on every successful run; the watchdog alerts if no success in `STALE_THRESHOLD_HRS = 6` during weekday business hours (local hour 11–21, after the day's first run+watchdog cycle). This catches ANY silent failure regardless of cause. Throttled via `data/staleness_alerted.json` to at most once per 6h. **Lesson from the 2026-06-08 outage:** every safety net watched for a specific failure type (crash, expired session, missed fire) and a plain network outage slipped through all of them — watch for missing *success*, not enumerated failures.
